...
def ExtractChanges(location, serverGen, cfg):
    #wrapper for SQL/AGOL extract changes functions
    if(location['type'] == 'SDE'):
        connection = sql.Connect(location['hostname'], location['database'], cfg.SQL_username, cfg.SQL_password)
        registration_id = sql.GetRegistrationId(connection, location['featureclass'])
        return sql.ExtractChanges(connection, registration_id, location['featureclass'], serverGen)

def ApplyEdits(location, cfg, json_dict):
    #wrapper for SQL/AGOL extract changes functions
    
    if(location['type'] == 'SDE'):
        connection = sql.Connect(location['hostname'], location['database'], cfg.SQL_username, cfg.SQL_password)
        registration_id = sql.GetRegistrationId(connection, location['featureclass'])
        sql.ApplyEdits(connection, registration_id, location['featureclass'], json_dict)
